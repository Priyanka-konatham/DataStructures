/*
 * Treeland is a country with n cities and n-1 roads. There is exactly one path between
 *  any two cities.

 * The ruler of Treeland wants to implement a self-driving bus system and asks tree-loving 
 * Alex to plan the bus routes. Alex decides that each route must contain a subset of 
 * connected cities; a subset of cities is connected if the following two conditions are true:

 * There is a path between every pair of cities which belongs to the subset.
 * Every city in the path must belong to the subset.
 * Each self-driving bus will operate within a connected segment of Treeland. 
